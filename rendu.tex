...
D(i,j)= min\left\{
    \begin{array}{ll}
        ...
        D(i',j) +  c_{del} * (i-i') :  i' \in [0..i[ \\
        D(i,j') +  c_{ins} * (j-j') :  j' \in [0..j[
    \end{array}
\right.
$$
        ...
        Sinon:
            ...
            Sinon :
                ...
                Pour i' allant de 0 a i:
                    Si D[i',j] + (i-i') * $c_{del}$ < c:
                        c $\gets$ D[i',j] + (i-i') * $c_{del}$
                fin pour
                
                Pour j' allant de 0 a j:
                    Si D[i,j'] + (j-') * $c_{ins}$ < c:
                        c $\gets$ D[i,j'] + (j-j') * $c_{ins}$
                fin pour
                
                Si D[i-1,j-1] + $c_{sub}(x_{i},y_{j})$ < c:
...
On a trois boucles imbriquées allant jusqu'à n/m. Sa complexité temporelle est donc de l'ordre de $O(max(n,m)^{3})$.
